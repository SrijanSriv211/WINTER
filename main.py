...
si.execute("search,youtube", ['"I made my own game engine"'])
...
for text, expec_tag in test:
    enctxt = nltk_utils.one_hot_encoding(text, model["vocab"]) # encoded text
    tag, conf = i.predict(enctxt, model["classes"])
