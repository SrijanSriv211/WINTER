...
model = rnn.sample("bin\\models\\skills.pth")
model.load()
...
for i in test:
    print(f"> {i}")
    out = one_hot_encoding(tokenizer.encode(i), list(tokenizer.vocab.keys()))
    model.predict(out)
    print()
...
# model = write.Sample("bin\\models\\GATw\\GATw.pth")
# model.load()
...
# feature = ExecEngine("data\\skills.json")
